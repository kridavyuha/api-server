...
func (ns *NotificationService) GetNotifications(user_id int) ([]Notification, error) {
	...
	for _, notif := range details {
		...
		if notif.EntityTypeId == 1 || notif.EntityTypeId == 2 {
			...
			err := ns.DB.Raw("SELECT T.shares, T.price, T.transaction_time, P.player_name from transactions as T join players as P on P.player_id = T.player_id  where user_id = ?", user_id).Scan(&transaction).Error
			if err != nil {
				return nil, err
			}
